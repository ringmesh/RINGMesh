...
there is nothing to do when compiling for OSX :-) 
...
#ifdef LINUX
// [Bruno] fpu_control.h does not exist under Android
#ifndef __ANDROID__
...
REAL exactinit()
{
...
#else /* not SINGLE */
     // Setting the precision generates the following runtime assertion failure:
     // f:\dd\vctools\crt_bld\self_64_amd64\crt\prebuild\tran\amd64\ieee.c(109) : Assertion failed: (mask&~(_MCW_DN|_MCW_EM|_MCW_RC))==0
     // 
     // See comment in _control87 documentation (http://msdn.microsoft.com/fr-fr/library/e9b52ceh%28v=vs.90%29.aspx)
     ...
     _FPU_SETCW(cword);
     // [Bruno]:
     // Don't know what to do under Android,
     // but normally default rounding mode is
     // OK (to be checked).
#endif
#endif /* LINUX */
