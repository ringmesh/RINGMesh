...
int loadMesh(pMesh mesh) {
  ...
  if ( !ptr ) {
    strcat(data,".meshb");
    if ( (inm = GmfOpenMesh(data,GmfRead,&mesh->ver,&mesh->dim)) == 0 ) {
      ptr = strstr(data,".mesh");
      *ptr = '\0';
      strcat(data,".mesh");
      if ( (inm = GmfOpenMesh(data,GmfRead,&mesh->ver,&mesh->dim)) == 0 ) {
  ...
  else if ( (inm = GmfOpenMesh(data,GmfRead,&mesh->ver,&mesh->dim)) == 0 ) {
    if ( ddebug )  fprintf(stderr,"  ** %s  NOT FOUND.\n",data);
    return(-1);
  }
...
int saveMesh(pScene sc,pMesh mesh,char *fileout,ubyte clipon) {
  ...
  ver = GmfFloat;
  if ( (outm = GmfOpenMesh(fileout,GmfWrite,ver,mesh->dim)) == 0 ) {
...
int loadSol(pMesh mesh,char *filename,int numsol) {
  ...
  if ( ptr )  *ptr = '\0';
  strcat(data,".solb");
  if ( (inm = GmfOpenMesh(data,GmfRead,&ver,&dim)) == 0 ) {
    ptr  = strstr(data,".sol");
    *ptr = '\0';
    strcat(data,".sol");
    if ( (inm = GmfOpenMesh(data,GmfRead,&ver,&dim)) == 0 )
