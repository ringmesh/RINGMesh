...
static void displayPoint(pScene sc,pMesh mesh,Color *c) {
  ...
  glPointSize(5);
  glBegin(GL_POINTS);
  for (k=1; k<=(unsigned int)mesh->np; k++) {
    ppt = &mesh->point[k];
...
GLuint pickingScene(pScene sc,int x,int y,int ident) {
  ...
  if ( !mesh->ne ) {
    if ( item <= (unsigned int)(mesh->np) ) {
      refitem = item;
      reftype = LPoint;
    }
  }
  else if ( item <= (unsigned int)(mesh->nt) ) {
    refitem = item;
    reftype = LTria;
  }
  else if ( item <= (unsigned int)(mesh->nt+mesh->nq) ) {
    refitem = item - mesh->nt;
    reftype = LQuad;
  }
  else if ( item <= (unsigned int)(mesh->nt+mesh->nq+mesh->ntet) ) {
    refitem = item - (mesh->nt+mesh->nq);
    reftype = LTets;
  }
  else if ( item <= (unsigned int)(mesh->nt+mesh->nq+mesh->ntet+mesh->nhex) ) {
